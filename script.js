...
function startGame() {
  if (game_state !== 'Play') {
    ...
    // Reset and start fly + catch on loop
    flySound.loop = true;
...
function endGame() {
  ...
  loseSound.removeEventListener('ended', playLose);
  loseSound.addEventListener('ended', playLose);
  playLose(); // First play immediately
}
...
function moveFollowers() {
  ...
  function updateFollowers() {
    ...
    target1X = Math.max(0, Math.min(window.innerWidth - 60, target1X));
